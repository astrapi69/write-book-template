...
def wired_module(temp_project: Path):
    ...
    # Copy scripts
    _copy_script(repo_root / "scripts" / "full_export_book.py", temp_project / "scripts" / "full_export_book.py")
    _copy_script(repo_root / "scripts" / "convert_to_absolute.py", temp_project / "scripts" / "convert_to_absolute.py")
    _copy_script(repo_root / "scripts" / "convert_to_relative.py", temp_project / "scripts" / "convert_to_relative.py")

    feb = _import_from_path("full_export_book", temp_project / "scripts" / "full_export_book.py")
    return feb


def test_pipeline_runs_convert_scripts_in_correct_order(wired_module, temp_project, monkeypatch, capsys):
    """
    ...
    def fake_compile_book(fmt, order, cover_path=None, force_epub2=False, lang="en", custom_ext=None):
    ...
    monkeypatch.setattr(feb, "IMG_SCRIPT", str(temp_project / "scripts" / "convert_img_tags.py"))
    ...
    filtered = [c for c in calls if c[0] in ("convert_to_absolute.py", "convert_to_relative.py", "convert_img_tags.py")]
    assert filtered == expected, f"Call order mismatch.\nExpected: {expected}\nGot:      {filtered}"
...
def test_language_resolution_cli_overrides_metadata(wired_module, temp_project, monkeypatch):
    feb = wired_module
    seen = {}

    def capture_compile(fmt, order, cover_path=None, force_epub2=False, lang="en", custom_ext=None):
        seen["lang"] = lang
...
def test_language_resolution_uses_metadata_when_cli_missing(wired_module, temp_project, monkeypatch):
    feb = wired_module
    seen = {}
    ...
    feb.main()
    assert seen.get("lang") == "de", "Should use language from metadata.yaml when CLI --lang not provided"


def test_run_script_bubbles_up_subprocess_errors(wired_module, temp_project, monkeypatch):
    feb = wired_module
