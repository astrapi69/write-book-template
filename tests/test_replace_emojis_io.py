...
def test_process_file_overwrite(tmp_path: Path):
    ...
    mapping = {"🔥": "+++"}
    s, n, dest = process_file(src, overwrite=True, suffix="-final.md", mapping=mapping, encoding="utf-8")
    assert s == src
    assert n == 1
    assert dest == src
...
def test_process_file_copy_only_if_changed(tmp_path: Path):
    ...
    mapping = {"🔥": "+++"}
    s, n, dest = process_file(src, overwrite=False, suffix="-final.md", mapping=mapping, encoding="utf-8")
    assert n == 0
    assert dest is None
