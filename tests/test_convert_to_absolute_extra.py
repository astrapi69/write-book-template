...
def test_relative_with_dot_and_dotdot(tmp_path: Path):
    ...
    rel2 = "./" + os.path.relpath(img, start=md.parent)        # ./../../assets/imgs/a.png (still valid)
...
def test_walk_three_default_dirs_with_mixed_content(tmp_path: Path):
    ...
    write(md1, f'![a]({rel1})')
    write(md2, f'![b]({rel2}) "trailer text"')  # trailing text after tag to ensure we match minimally
