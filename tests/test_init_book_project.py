...
def test_init_writes_nested_isbn_and_updates_files(tmp_path: Path):
    ...
    (tmp_path / "scripts" / "full_export_book.py").write_text(DUMMY_FULL_EXPORT, encoding="utf-8")
    ...
    (tmp_path / "pyproject.toml").write_text(toml.dumps(pyproject_min), encoding="utf-8")
    ...
    assert updated_pyproject["tool"]["poetry"]["description"] == "A book about intangible wealth."

    # Assert: full_export_book.py updated (placeholders replaced)
    fe_content = (tmp_path / "scripts" / "full_export_book.py").read_text(encoding="utf-8")
