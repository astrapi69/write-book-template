...
# Note: For print PDF, the TOC is auto-generated by LaTeX with page numbers,
# so we don't include a manual TOC file here
...
def filter_section_order_for_epub(section_order: list[str]) -> list[str]:
    """
    Remove manual TOC files from section order for EPUB builds.
...
def filter_section_order_for_print_pdf(section_order: list[str]) -> list[str]:
    """
    Remove manual TOC files from section order for print PDF builds.

    LaTeX/Pandoc will generate the TOC automatically with page numbers
    using the --toc flag, which is the standard for print books.
    """
    filtered = [s for s in section_order if s not in EPUB_SKIP_TOC_FILES]
    return filtered
...
def compile_book(
    ...
    # For EPUB: filter out manual TOC files (Pandoc generates TOC automatically)
    if format == "epub":
        effective_order = filter_section_order_for_epub(section_order)
        if len(effective_order) < len(section_order):
            print(
                "ℹ️  EPUB: Skipping manual TOC. Pandoc will generate TOC automatically."
            )
    # For print PDF: filter out manual TOC files (LaTeX generates TOC with page numbers)
    elif format == "pdf" and is_print_build:
        effective_order = filter_section_order_for_print_pdf(section_order)
        if len(effective_order) < len(section_order):
            print(
                "ℹ️  Print PDF: Skipping manual TOC. LaTeX will generate TOC with page numbers."
            )
    else:
    ...
    if format == "epub":
        ...
        # Add automatic TOC generation for EPUB
        # This creates proper cross-file links that pass epubcheck validation
        pandoc_cmd.extend(
            [
    ...
    if format == "pdf":
        pandoc_cmd.extend(
            [
                "--pdf-engine=lualatex",  # Options: xelatex, lualatex, pdflatex
                "-V",
        ...
        # For print builds (paperback/hardcover): add auto-generated TOC with page numbers
        if is_print_build:
            pandoc_cmd.extend(
                [
                    "--toc",  # Generate table of contents
                    f"--toc-depth={toc_depth}",  # TOC depth (default: 2)
                    "-V",
                    "toc-title=Table of Contents",  # Customize TOC title if needed
                ]
            )
            print(
                f"ℹ️  Print PDF: Auto-generating TOC with page numbers (depth={toc_depth})"
            )
