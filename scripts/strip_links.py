...
def strip_links_in_text_no_code(text: str) -> Tuple[str, int]:
    """Helper for non-code segments only."""
    return _strip_links_segment(text)
...
def parse_args(argv: Iterable[str] | None = None) -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Strip inline Markdown links [text](url) -> text (skips images and code)."
    )
    ap.add_argument("--file", type=Path, default=DEFAULT_TOC,
    ...
    ap.add_argument("--suffix", default="-nolinks.md",
