...
def translate_markdown_file(
    ...
    for block in blocks:
        if block.strip():
            try:
                translated = (
                    translate_text(block.strip(), source_lang, target_lang)
                    if not dry_run
                    else block
                )
...
def main():
    parser = argparse.ArgumentParser(
        description="Translate markdown files using DeepL Free API."
    )
    parser.add_argument(
        "--source-lang",
        type=str,
    ...
    parser.add_argument(
        "--base-dir", type=str, help="Directory containing Markdown files"
    )
    parser.add_argument("--file", type=str, help="Path to a specific Markdown file")
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Run without writing files or updating skiplist",
    )
            ...
            if new_entries:
                ...
                with open(SKIPLIST_PATH, "w", encoding="utf-8") as f:
                    for path in combined_entries:
                        ...
                        f.write(path + "\n")
                print(
                    f"📝 Skiplist updated with {len(new_entries)} new entries (total {len(combined_entries)})."
                )
            else:
