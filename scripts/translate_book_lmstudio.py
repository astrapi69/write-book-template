...
def translate_text(
    text: str, source_lang: str = "en", target_lang: str = "es", model: str = "llama3"
) -> str:
    """
        ...
        if block.strip():
            try:
                translated = translate_text(
                    block.strip(), source_lang, target_lang, model
                )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Translate markdown files using LM Studio REST API."
    )
    parser.add_argument(
        "--source-lang",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Target language code (e.g., es, fr)",
    )
    parser.add_argument(
        "--base-dir",
        type=str,
